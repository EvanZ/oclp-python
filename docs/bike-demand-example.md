...
output called `metrics`. The `prefix` makes the MLflow keys readable. For
repeated `train_fold` Executions, the declaration
`MlflowMetrics(output_port="metrics", prefix="temporal-fold",
dimensions=("fold_number",))` adds the declared Execution parameter to the
key, producing names such as `temporal-fold.fold_number-2.rmse` and preventing
MLflow's immutable metric namespace from conflating folds. Candidate evaluation
and holdout scoring run once, so their declarations need no dimensions.
