...
class RunTemplate:
    ...
    def profile_for(
        ...
        *,
        ...
        The workflow declaration supplies the default name. An integration may
        explicitly choose a concrete application-owned label while preserving
        the declared adapters, evidence policy, and run UUID.
        """
...
def observe_run(
    ...
    *,
    ...
    the workflow declaration and concrete UUID. ``run_name`` can explicitly
    replace the declaration's display-name default without changing the
    declared adapters, evidence policy, or run identity.
    ``finalize_decorated_artifact_sets``
    ...
    if existing is not None and existing != generated:
        raise ValueError(
            "observe_run derives profiles.run from the declared workflow, "
            "explicit run_name, and concrete run UUID; do not override it"
        )
